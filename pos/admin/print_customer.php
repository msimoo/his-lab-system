...
if (!$customer_id) {
    die(__('invalid_customer') ?? 'Invalid Customer ID');
}

$customerRes = $mysqli->prepare("SELECT customer_id, customer_name, customer_phoneno, customer_email, customer_details FROM rpos_customers WHERE customer_id = ? LIMIT 1");
$customerRes->bind_param('i', $customer_id);
$customerRes->execute();
$customer = $customerRes->get_result()->fetch_assoc();
if (!$customer) {
    die(__('customer_not_found') ?? 'Customer not found');
}

$accSql = "SELECT account_id, invoice_id, total_amount, paid_amount, remaining_amount, status, created_at, updated_at FROM rpos_customer_accounts WHERE customer_id = ?";
$accParams = [$customer_id];
if ($account_id !== '') {
...
$historySql = "SELECT history_id, account_id, invoice_id, payment_amount, previous_paid, new_paid, previous_remaining, new_remaining, status, note, created_by, created_at FROM rpos_customer_account_history WHERE customer_id = ?";
$historyParams = [$customer_id];
if ($account_id !== '') {
...
<head>
    ...
    <title><?php echo __('print_customer_report') ?? 'Customer Report'; ?> - <?php echo htmlspecialchars($customer['customer_name']); ?></title>
        ...
        <div class="report-card">
            <div class="row slip-header">
                ...
                <div class="col-sm-6 text-right">
                    <h3 style="font-weight:300;color:#888;"><?php echo __('customer_report') ?? 'Customer Report'; ?></h3>
                    <div><?php echo __('customer_name') ?? 'Customer'; ?>: <?php echo htmlspecialchars($customer['customer_name']); ?></div>
                    <div><?php echo __('phone') ?? 'Phone'; ?>: <?php echo htmlspecialchars($customer['customer_phoneno']); ?></div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm">
                    <thead><tr><th><?php echo __('invoice_id') ?? 'Invoice ID'; ?></th><th><?php echo __('total_amount') ?? 'Total'; ?></th><th><?php echo __('paid_amount') ?? 'Paid'; ?></th><th><?php echo __('remaining_amount') ?? 'Remaining'; ?></th><th><?php echo __('status') ?? 'Status'; ?></th><th><?php echo __('created_at') ?? 'Created'; ?></th><th><?php echo __('updated_at') ?? 'Updated'; ?></th></tr></thead>
                    <tbody>
                        <?php if (empty($accounts)) : ?>
                            <tr><td colspan="7" class="text-center"><?php echo __('no_accounts_found') ?? 'No customer accounts found.'; ?></td></tr>
                        <?php else: ?>
                            <?php foreach ($accounts as $acc): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($acc['invoice_id']); ?></td>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm">
                    <thead><tr><th><?php echo __('date') ?? 'Date'; ?></th><th><?php echo __('account_id') ?? 'Account ID'; ?></th><th><?php echo __('invoice_id') ?? 'Invoice ID'; ?></th><th><?php echo __('payment_amount') ?? 'Payment Amount'; ?></th><th><?php echo __('previous_paid') ?? 'Prev Paid'; ?></th><th><?php echo __('new_paid') ?? 'New Paid'; ?></th><th><?php echo __('previous_remaining') ?? 'Prev Remaining'; ?></th><th><?php echo __('new_remaining') ?? 'New Remaining'; ?></th><th><?php echo __('status') ?? 'Status'; ?></th><th><?php echo __('note') ?? 'Note'; ?></th><th><?php echo __('created_by') ?? 'By'; ?></th></tr></thead>
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($history as $h): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($h['invoice_id']); ?></td>
