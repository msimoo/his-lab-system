...
$payid = $payid ?? bin2hex(random_bytes(6));
$mpesaCode = $mpesaCode ?? substr(str_shuffle("Q1W2E3R4T5Y6U7I8O9PLKJHGFDSAZXCVBNM"),1,10);
...
if (isset($_POST['pay'])) {
  ...
  if (empty($_POST["pay_code"]) || empty($_POST["pay_amt"]) || empty($_POST['pay_method'])) {
    ...
    $pay_id = $_POST['pay_id'];
    $store_id = $_POST['store_id'] ?? $_SESSION['selected_store'] ?? '';
    $order_status = $_POST['order_status'] ?? 'Paid';

    // Insert captured information to a database table
    $postQuery = "INSERT INTO rpos_payments (pay_id, pay_code, order_code, customer_id, pay_amt, pay_method, store_id) VALUES(?,?,?,?,?,?,?)";
    ...
    $rc = $postStmt->bind_param('sssssss', $pay_id, $pay_code, $order_code, $customer_id, $pay_amt, $pay_method, $store_id);
          ...
          <div class="card shadow">
            ...
            <div class="card-body">
              <form method="POST"  enctype="multipart/form-data">
                ...
                <input type="hidden" name="store_id" value="<?php echo htmlspecialchars($order->store_id ?? $_SESSION['selected_store'] ?? ''); ?>">
